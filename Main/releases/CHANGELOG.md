...
- Fixed 8-digit grid handling so KP is ignored when the grid already contains keypad-level precision.
- Disabled KP input fields in the Mission Inputs UI when 8-digit gun or target grids are detected.
- Preserved 6-digit grid + KP behavior for coarse grid workflows.
- Preserved the v4.1 true floating-module workspace where Readout, Notes, and Data Log can be popped out and removed from the main workspace.
- Preserved v4.1.2 update checker polish where Manifest Source, Status, and Details display as non-editable data panels.
- Continued public manifest/update checker support through `releases/latest.json`.
- Automatic installation is still intentionally deferred to a future updater-helper pass.

## v4.1.2-beta

- Polished the update checker UI so Manifest Source, Status, and Details are shown as non-editable data panels instead of editable-looking text boxes.
- Preserved manifest checking, version comparison, ZIP download, and optional SHA256 verification.

## v4.1.1-beta

- Added the first C# / Avalonia public update checker workflow.
- Added `Check Updates` in the app header.
- Added public manifest support through `releases/latest.json`.
- Added local fallback manifest support through `data/update_manifest.json`.
- Added update ZIP downloading to `%APPDATA%\BravoFiresFDC\updates`.
- Added optional SHA256 verification.
- Automatic installation was intentionally deferred.

## v4.1.0-rc1

- Restored true floating-module workspace behavior.
- Fire Solution Readout, Mission Notes, and Fire Mission Log can pop out as independent floating modules.
- Popped-out modules are removed from the main workspace.
- Mission Inputs can expand across the full main workspace when Readout, Notes, and Data Log are floating.
- Reset Layout docks floating modules and restores the default workspace.

## v4.0.6-beta

- Established the first functional C# / Avalonia migration baseline.
- Fixed startup initialization and control wiring issues.
- Added startup diagnostics and flexible data/legal file loading.
- Confirmed the app builds, publishes, and launches successfully as a self-contained Windows x64 executable.

---

# Previous Python/Tkinter Release Line


## v3.0.7-beta

- Added a **Download Update ZIP** button to the Update Available window.
- Update packages now download to the user's Downloads folder.
- Added support for direct GitHub release asset URLs from `update_manifest.json`.
- Added fallback resolution from GitHub release tag pages to downloadable release assets.
- Kept **Open Release Page** and **Open Changelog** actions available.
- Automatic file replacement/install behavior is not included yet and remains planned for a later V3.x build.
- No calculation or layout behavior changed.


## v3.0.6-beta

- Reorganized the private/source package into labeled folders for cleaner development storage.
- Added `src/` for Python source files.
- Added `assets/` for logos and icon files.
- Added `data/` for `ballistics_data.json` and `update_manifest.json`.
- Added `legal/` for license, EULA, privacy policy, terms, and third-party notices.
- Added `docs/` for packaged README and commit description notes.
- Removed `run_bravo_fires_fdc.bat` because it was an Alpha-era carryover.
- Updated `build_windows_exe.bat` to build from the structured folder layout.
- Updated resource loading so packaged/source runs can find files from the new folder layout.
- Added build-script cleanup for the downloaded v3.0.6 package ZIP when it is found in the repo folder or parent folder.
- No calculation or layout behavior changed.
